...
// it). The hook runs on the calling goroutine; keep it cheap and must not panic (a
// panic propagates to the caller — the wrapper does not recover). When nil, the cost
