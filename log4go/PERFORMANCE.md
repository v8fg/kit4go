...
| **Kafka（noop producer）** | **2582** | **~387K** | 1336 | 27 | 生产发 kafka（JSON buildPayload 主导，见下）|
...
Kafka `Write` 热路径 = `buildPayload`（JSON 序列化 record → map → json.Marshal，~2582ns / 27 allocs）+ 入队（~100ns）。JSON 序列化占 95%+ 开销。对比 File async（219ns，仅 copy record + 入队），Kafka 慢约 12x 全因 JSON。优化方向：复用编码器 buffer、避免 map 中间层（已在 JSON 格式 FormatJSON 路径解决：`r.jsonBytes` 预序列化一次，Kafka buildPayload 可直接用）。
...
| **Kafka（noop producer）** | 0.00 | 0.00 | **55** | 4 | JSON buildPayload 高 alloc → GC 频 |
...
- **Kafka NumGC=55**：JSON buildPayload 每 record 27 allocs，100K 条触发频繁 GC。生产用 FormatJSON 预序列化（r.jsonBytes）可降 ~10x allocs（见 §12）。
